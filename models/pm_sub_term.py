...
class SubTerm(models.Model):
    ...
    term_id = fields.Many2one(comodel_name='pm.term', string="Term",
                              ondelete='restrict')  # Reference to the parent term

    product_ids = fields.Many2many(comodel_name='product.template', string="Products",
                                   ondelete='restrict')  # List of products used in the sub-term
    ...
    contractor_subterm_ids = fields.One2many(comodel_name='pm.contractor.subterm', inverse_name='sub_term_id',
                                             ...
                                             ondelete='restrict')  # List of contractor sub-terms
    ...
    def _check_dates(self):
        ...
        for rec in self:
            if not rec.term_id:
                raise ValidationError('Sub-Term must be within a Term')

            if rec.end_date < rec.start_date:
                raise ValidationError('End Date must be greater than Start Date')

            if rec.start_date < rec.term_id.start_date or rec.end_date > rec.term_id.end_date:
                raise ValidationError('Start Date and End Date must be within the Term')
...
class PmContractorSubTerm(models.Model):
    ...
    # Name of the contractor sub-term composed of the sub-term name and the contractor names
    ...
    # Quantity of products to be added to the progress by the contractor
    ...
    def add_qty_in(self):
        ...
        Raises:
            ValidationError: If the contractor sub-term is not started or if the quantity in is greater than the remaining quantity.
        """
        try:
            if self.state != 'started':
                raise ValidationError('Contractor SubTerm must be started')

            to_make = self.qty - self.progress
            if self.qty_in > to_make:
                raise ValidationError(f'Quantity In must be at most {to_make}')
    ...
    def _check_dates(self):
        ...
        for rec in self:
            ...
            if rec.start_date < rec.sub_term_id.start_date or rec.end_date > rec.sub_term_id.end_date:
                raise ValidationError('Start Date and End Date must be within the Sub-Term')
    ...
    def _check_progress(self):
        ...
        for rec in self:
            if rec.progress < 0 or rec.progress > rec.qty:
                raise ValidationError(f'Progress must be between 0 and {rec.qty}')
    ...
    def _onchange_qty(self):
        ...
        if self.qty > free_qty:
            raise ValidationError(f'Quantity must be at most {free_qty}')
